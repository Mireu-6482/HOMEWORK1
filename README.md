...
생성자 함수와 프로토타입을 이용한 메서드 정의 방식을 보여주는 기본 예제

## 6-2

생성자 함수와 그 인스턴스의 프로토타입 체인 구조 및 console.dir()을 통한 객체 탐색

## 6-3

constructor 프로퍼티를 통해 생성자 정보를 참조하는 방식과 생성자 재사용을 보여주는 예제

## 6-4

기본 자료형 및 객체들의 constructor를 변경해도 instanceof는 prototype 체인 기준으로 판단된다는 것을 보여주는 예제

## 6-5

constructor, __proto__, object.getprototypeOf를 활용해 동일한 생성자를 다양한 방식으로 참조 가능함을 보여주는 예제

## 6-6

인스턴스 객체에서 프로토타입 메서드를 오버라이드(덮어쓰기)하는 방법과 동작 확인

## 6-7

arr.hasOwnProperty(2)는 arr[2]가 직접 정의된 값인지 확인하는 코드로, 배열의 고유 속성 여부 판단을 보여줌.

## 6-8

toString() 메서드의 재정의와 call()을 이용한 다양한 프로토타입 메서드 호출 방법을 비교

## 6-9

Object.prototype에 메서드를 추가하여 모든 객체에서 사용할 수 있는 유틸성 메서드 구현 (getEntries)를 보여줌. 원시값에도 메서드가 호출 가능한 이유는 박싱(Wrapper Object) 때문임.

## 6-10

배열처럼 작동하는 객체(this[i], this.length)를 만드는 예제로, 유사 배열 객체를 직접 구현한 방식.

## 7-1

인스턴스 메서드 vs 정적 메서드의 차이와 사용법

## 7-2

생성자 함수에 배열 prototype 설정 (유사 배열 객체)

## 7-3

프로토타입 배열로 설정 시 push 사용 가능, length 삭제 후의 동작 확인

## 7-4

프로토타입 자체를 배열로 설정했을 때의 충돌 사례

## 7-5

두 생성자 함수(Rectangle, Square) 각각 정의 된 경우 - 중복 코드

## 7-6

Square가 내부적으로 Rectangle과 동일한 구조를 가짐 (코드 중복 해결)

## 7-7

Rectangle의 생성자를 Square에서 호출 + 프로토타입 연결 시도

## 7-8

상속 함수 구현 - 프로토타입 연결 후 인스턴스 프로퍼티 제거, freeze 사용

## 7-9

브릿지 패턴 사용 - 부모의 프로토타입 공유 (효율적 상속)

## 7-10

Object.create를 활용한 프로토타입 상속 (가장 권장되는 방식)

## 7-11

SuperClass 인스턴스를 SubClass.prototype으로 직접 설정 후 프로토타입 속성을 삭제해 초기화하는 원시적 상속 방식

## 7-12

상속 구조를 깨지 않기 위해 Bridge라는 중간 함수를 사용해 SuperClass.prototype을 공유하지 않고 상속하는 방식

## 7-13

Object.create()를 이용해 SuperClass.prototype을 안전하게 상속하는 현대적이고 간결한 방식

## 7-14

super 메서드 수동 구현 (ES5 스타일에서 super 키워드처럼 구현)

## 7-15

ES5 vs ES6 클래스 - 정적 메서드와 인스턴스 메서드 정의 비교

## 7-16

ES6 상속 클래스에서 super()로 부모 생성자 호출 및 메서드 오버라이드